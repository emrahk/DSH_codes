pro plot_bat_maxi_23, drange, ps=ps, fname=namef, shobsdates=shobsdates, $
                      lowe=lowe, batmaxi=batmaxi
...
IF NOT keyword_set(batmaxi) THEN batmaxi=0
...
IF NOT lowe THEN oplot,!x.crange, [0.4,0.4],line=2
...
ENDIF ELSE BEGIN
   ...
   nytit='MAXI 4-10/2-4 keV hardness)'
   rat=m410[ma]/m24[ma]
   erat=e410[ma]/m410[ma]
   
   ymax=1.8
   nyr=[0.6,ymax]

   ploterror, mjdm[ma]-57000, rat, rat*erat, yr=nyr, /xstyle, /ystyle, $
           /nohat, $
           chars=cs, psym=8, xr=nxr, background=backc, $
           ytitle=nytit,axiscolor=axc
ENDELSE
