pro wrap_simprof_cont_swift21_mdreid, inpar, ps=ps, fname=namef, ylog=logy, cloudpar=parcloud, outsbp=sbpout, syserr=errsys, totchi2=chi2tot, silent=silent
...
;
  ...
  IF NOT keyword_set(namef) THEN namef='simprof_cont_swiftnew_mdreid.eps'
...
sed_data=read_ascii('DATA/J1634-473_g_lc_1day_all.dat')
mjdm=sed_data.field1[0,*]
m24=sed_data.field1[3,*]
e24=sed_data.field1[4,*]
...
ploterror,mjdm-55000.,m24,e24,psym=4,/nohat,xr=[4460,4540],$
          xtitle='Time (MJD-55000 days)',$
          ytitle='ph cm!E-2!N s!E-1!N',charsize=1.3,yr=[0,1.]



;oploterror,mjdm,m410,e410,psym=5,/nohat
oplot,[t21d,t21d]-55000.,[0.,1.],color=0,line=2,thick=2

;arrow, chand-50000., 0.3, chand-50000., 1E-5, /data, color=0

t0=59464.5  ;start of outburst
tl=59511.5  ;end of dates that we use maxi data

delt=tl-t0
;oplot,[t0,t0]-50000.,!y.crange,color=0    ;show if necessary
;oplot,[tl,tl]+9-50000.,!y.crange,color=0  ;show if necessary

xx=where((mjdm GT t0) AND (mjdm LE tl))
...
oplot,time+t0-55000.,F,color=0


;stop
;from MJD 59505 to t21d add linear decay
nt=findgen(20)
time2=59505.+nt
F2=0.6-nt*0.035
tim2add=where((time2 gt tl) AND (time2 lt t21d))
oplot,4505+nt,0.6-nt*0.035,color=0
...
sim_prof, x, D, alpha, beta, meanE, Foft, t21d-t0, totprof1, clth=thcl, $
...
FOR i=1, parcloud.ncloud-1 DO BEGIN
   ...
   sim_prof, x, D, alpha, beta, meanE, Foft, t21d-t0, totprofi, clth=thcl, $
             /noplot, silent=silent
   sbpout[i,*]=totprofi
...
plot, findgen(600),sbptot, color=50, line=2, thick=4, yr=[8e-10,1.2e-6], $
      xr=[0,600], xtitle='Radius (arcsec)', $
          ytitle='ph cm!E-2!N s!E-1!N asec!E-2!N',charsize=1.3,ylog=logy
